    ...
    <main class="container">
        <section class="row">
            <div class="col-md-12 my-4">
                ...
                <h2 class="mt-5 mb-4">Modifica la Categoria</h2>
            </div>
            <div class="col-md-5">
                <form action="{{ route('categories.update', $category->id) }}" method="POST">
                    @csrf
                    @method('PUT') <!-- Indica che questo è un metodo PUT per aggiornare -->
                    <div class="mb-3 form-floating">
                        <input type="text" name="name" class="form-control" id="title" placeholder="Romanzo, Saggio,..."
                               value="{{ old('name', $category->name) }}" required>
                        <label for="title">Nome della Categoria</label>
                    </div>
                    <div class="pt-4 border-top">
                        <button type="submit" class="btn btn-primary me-3">Aggiorna la Categoria</button>
